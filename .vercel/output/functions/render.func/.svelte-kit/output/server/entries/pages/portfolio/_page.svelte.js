...
<div class="${'content'}">${``}
	...
	<div class="${'tabs svelte-es56d3'}">
		...
		<div class="${'design-tab tab ' + escape(designPos, true) + ' svelte-es56d3'}"><h2 class="${'tab-label svelte-es56d3'}">design</h2>
			...
			<div class="${'tab-page svelte-es56d3'}"><div class="${'tab-page-content svelte-es56d3'}">
					<div class="${'buttons'}">
						${each(
							Object.keys(designTags).filter((k) => designTags[k]),
							(tag) => {
						...
						<div>${validate_component(Tag, 'Tag').$$render(
							$$result,
							{ className: 'clear' },
							{},
							{
					...
					<div class="${'content-container'}">
						${each(designItems, (category) => {
							return `${
								noneSelected() || designTags[category.tag]
									? `${each(category.rows, (row) => {
											return `<div class="${escape(category.tag, true) + ' content-row svelte-es56d3'}">${each(
												row,
												(item) => {
													return `<div class="${'img-container svelte-es56d3'}">${
														isMobile
															? `${
																	currentMobileDescription === item.title
																		? `<div class="${'description mobile svelte-es56d3'}"><h2 class="${'svelte-es56d3'}">${escape(item.title)}</h2>
															...
															<p style="${'margin-top: 0.75rem'}" class="${'svelte-es56d3'}">${escape(item.description)}
																${
																	'link' in item
																		? `<span class="${'more-link-mobile svelte-es56d3'}">More <a${add_attribute('href', item.link, 0)}>here</a>.
																	</span>`
																		: ``
																}</p>
														</div>`
													...
													<img${add_attribute('src', item.src, 0)}${add_attribute('alt', item.title, 0)} class="${'svelte-es56d3'}">`
															: `<div class="${'web-item-container'}"><div class="${'description svelte-es56d3'}" style="${'position: fixed; left: ' + escape(m.x + 5, true) + 'px; top: ' + escape(m.y + 5, true) + 'px'}"><h2 class="${'svelte-es56d3'}">${escape(item.title)}</h2>
															${validate_component(Tag, 'Tag').$$render(
																$$result,
																{ className: 'inactive' },
																{},
		...
		<div class="${'projects-tab tab ' + escape(projectsPos, true) + ' svelte-es56d3'}"><h2 class="${'tab-label svelte-es56d3'}">projects</h2>
			<svg viewBox="${'0 0 263 108'}" fill="${'none'}" xmlns="${'http://www.w3.org/2000/svg'}" class="${'svelte-es56d3'}"><path d="${'M34.3647 18.5394L26.4733 35.1565C20.9981 46.6858 11.3657 53.6959 0.998535 53.6959V107.392H262.5V53.6959C252.318 53.6959 242.831 46.9326 237.304 35.7337L228.534 17.9622C223.007 6.76327 213.52 0 203.338 0H59.8395C49.4724 0 39.84 7.01005 34.3647 18.5394Z'}"></path></svg>
			<div class="${'tab-page svelte-es56d3'}"><div class="${'tab-page-content svelte-es56d3'}"><div class="${'buttons'}">${each(
				Object.keys(projectsTags).filter((k) => projectsTags[k]),
				(tag) => {
					return `<div>${validate_component(Tag, 'Tag').$$render(
						$$result,
						{
					...
					<div class="${'content-container'}">
						...
						${
							Object.keys(projectsTags).filter((k) => projectsTags[k]).length === 0
	...
	<div class="${'outro svelte-es56d3'}"><h2 class="${'svelte-es56d3'}">don&#39;t be shy...</h2>
		<div class="${'icons svelte-es56d3'}"><a sveltekit:prefetch href="${'https://www.linkedin.com/in/vivianwli/'}">${validate_component(
			Icon,
			'Icon'
		).$$render(
			$$result,
			...
			<a sveltekit:prefetch href="${'mailto:vivianwli.design@gmail.com'}">${validate_component(
				Icon,
				'Icon'
			).$$render(
				$$result,
